...
(defn send-log-entries [dst]
  ...
  (let [tape (:tape @data)
    ...
    (println (str"next-index-follower" dst "-" last-log-index "->" next-index-follower))
    (println "sending" (count entries-to-send))
    (comment (prn [dst
    ...
    (if (>= last-log-index next-index-follower) ;; TODO I think I can get rid of this, and use this fn to send heartbeats as well...
      ...
      (let [resp (send-rpc :rpc/append-entries
        ...
        (cond (:success resp)
              ...
              (do
                (println dst "decing and retrying, they have " (:my-latest resp), "we have " last-log-index)
                (swap! leader-state (fn [st]
                                      (assoc-in st [:next-index dst]
                                                (max 0
                                                     (min (or (inc (:my-latest resp)) 999999)
                                                          (dec (get-in st [:next-index dst])))))))
  ...
  (let [tape (:tape @data)]
    (cond (> @term (:term req))
          ...
          (do
            ...
            (let [new-tape (vec (concat
                                 (subvec tape 0 (inc prev-log-index))
                                 entries))]
